...
func TestSpider(t *testing.T) {
	t.Run("scheduler prioritizes requests and deduplicates fingerprints", func(t *testing.T) {
		...
		low := spiders.Request{URL: "https://example.com/list#ignored", Priority: 1}
		high := spiders.Request{URL: "https://example.com/detail", Priority: 10}
		duplicateLow := spiders.Request{URL: "https://example.com/list", Priority: 99}
		query := spiders.Request{URL: "https://example.com/search?b=2&a=1", Priority: 5}
		duplicateQuery := spiders.Request{URL: "https://example.com/search?a=1&b=2", Priority: 6}
	...
	t.Run("crawler routes sessions, follows requests, and collects results", func(t *testing.T) {
		...
		if err := sessions.Add("list", listSession, spiders.SessionOptions{Default: true}); err != nil {
			t.Fatalf("add list session: %v", err)
		}
		if err := sessions.Add("detail", detailSession, spiders.SessionOptions{Lazy: true}); err != nil {
			t.Fatalf("add detail session: %v", err)
		}

		var parseDetail spiders.Callback
...
func (s *fakeSession) Fetch(_ context.Context, request spiders.Request) (*goscrapling.Response, error) {
	s.mu.Lock()
