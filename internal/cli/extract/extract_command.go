...
import (
	...
	"github.com/TrebuchetDynamics/goscrapling/internal/cli/extract/output"
)
...
func executeExtractPlan(stdout io.Writer, plan extractPlan) error {
	...
	if plan.aiTargeted {
		response, err = output.AITargetedResponse(response)
		if err != nil {
			return err
		}
	}

	body, err := output.Render(response, plan.outputPath, plan.selector)
	if err != nil {
		return err
	}
	if err := output.WriteFile(plan.outputPath, body); err != nil {
		return err
	}
