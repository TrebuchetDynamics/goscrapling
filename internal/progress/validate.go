...
func validateBuilderLoopMeta(meta BuilderLoopMeta) []error {
...
func builderLoopMetaDeclared(meta BuilderLoopMeta) bool {
...
func validateItem(phaseKey, subphaseKey string, index int, item Item) []error {
	var errs []error
	prefix := func() string {
	...
	if item.Status == StatusInProgress && item.SliceSize == "umbrella" {
		add("in_progress row cannot use slice_size umbrella")
	}
	if item.Status == StatusComplete && item.Contract != "" && item.ContractStatus != "validated" {
...
func validateContractMetadata(prefix func() string, item Item) []error {
	var errs []error
	add := func(message string) {
		errs = append(errs, errors.New(prefix()+message))
	}
...
func validStatus(status Status) bool {
	return status == StatusComplete || status == StatusInProgress || status == StatusPlanned
}
