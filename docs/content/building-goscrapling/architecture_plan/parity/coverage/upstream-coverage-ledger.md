...
| `scrapling/core/shell.py`, `scrapling/core/_shell_signatures.py`, `scrapling/core/utils/_shell.py` | CLI shell and extract commands | `cmd/goscrapling`, `internal/cli` scripted shell evaluator, get/post/put/delete page shortcuts, help/namespace listing, pages history expressions, view artifact boundary, curl helper parsing/execution, planned browser shortcut boundaries, markdown/AI-targeted extract output cleanup, and non-mutating install guidance | Phase 5 | partial |
...
| `scrapling/core/ai.py` | MCP and AI docs | `integrations/mcp` deterministic tool schemas, fake static/browser seams, session lifecycle, screenshot content blocks, and CSS/main-content extraction | Phase 5 | partial |
| `scrapling/engines/static.py`, `scrapling/engines/toolbelt/custom.py`, `scrapling/engines/toolbelt/convertor.py` | Response object, static fetcher | `engines/toolbelt.Response`, `fetchers.Fetcher`, `fetchers.FetcherSession`, `fetchers.ConcurrentFetcher`, and root facade aliases | Phase 1 | partial |
| `scrapling/engines/_browsers/**` | Browser fetching | `engines/browser.BrowserFetcher`, `ChromedpBrowserEngine`, `BrowserSession` session/page pool lifecycle, explicit browser context/resource options, wait/action/capture controls, explicit stealth controls, unsupported Cloudflare/Turnstile challenge boundary, and root facade aliases | Phase 3 | partial |
...
| `scrapling/engines/toolbelt/navigation.py`, `scrapling/engines/toolbelt/ad_domains.py`, `scrapling/engines/constants.py` | Browser fetching, Proxy rotation | explicit browser proxy validation, context/resource controls, URL block pattern generation, and proxy error helpers | Phase 1, Phase 3 | partial |
| `scrapling/fetchers/requests.py` | Static fetcher | `fetchers.Fetcher`, `fetchers.FetcherSession`, `fetchers.ConcurrentFetcher`, and root facade aliases | Phase 1 | partial |
...
| `scrapling/fetchers/stealth_chrome.py` | Browser fetching, stealth controls | `engines/browser.BrowserStealthOptions` plus explicit unsupported Cloudflare/Turnstile challenge boundary | Phase 3 | partial |
| `scrapling/spiders/**` | Spider runtime | `spiders` package request/result/session/scheduler contracts, allowed-domain filtering, crawler concurrency controls, development response cache, blocked retry hooks, checkpoint pause/resume, robots.txt manager/delay directives, lifecycle hooks, streaming, item hooks, expanded stats, ItemList export, LinkExtractor, CrawlSpider/SitemapSpider templates, static/browser/stealth session adapters, and explicit proxy rotation hooks | Phase 4 | partial |
| `scrapling/cli.py` | CLI shell and extract commands | `cmd/goscrapling`, `internal/cli` install command, static extract, markdown/AI-targeted output, and fake-backed browser extract command seams | Phase 5 | partial |
| `Dockerfile`, `pyproject.toml`, `server.json` | Install, Docker, packaging, examples, and benchmarks | `cmd/goscrapling install`, install-packaging docs, integration metadata, README status map, public example tests, benchmark fixtures, and generated parity scorecard | Phase 5 | partial |
...
| `docs/api-reference/fetchers.md`, `docs/fetching/**` | Fetchers and browser fetching | `fetchers`, explicit static identity boundaries, Go-native concurrent static fetching, `engines/browser` fetchers/sessions/context/action/capture options, and root facade aliases | Phase 1, Phase 3 | partial |
...
| `docs/spiders/**` | Spider runtime | `spiders` package with fixture-backed core, allowed-domain filtering, concurrency/domain-delay controls, development response cache, blocked retry hooks, checkpoint pause/resume, robots.txt manager/delay directives, lifecycle hooks, streaming, item hooks, expanded stats, ItemList export, LinkExtractor, CrawlSpider/SitemapSpider templates, static/browser/stealth session adapters, and explicit proxy rotation hooks | Phase 4 | partial |
| `docs/cli/**` | CLI shell and extract commands | `cmd/goscrapling`, `internal/cli` install guidance, extract outputs, browser-mode wiring, curl helpers, and shell docs parity evidence | Phase 5 | partial |
| `docs/ai/mcp-server.md`, `docs/api-reference/mcp-server.md` | MCP and AI integration | `integrations/mcp` deterministic tool schemas, fake static/browser seams, session lifecycle, screenshot content blocks, and CSS/main-content extraction | Phase 5 | partial |
| `docs/benchmarks.md`, `docs/tutorials/**`, `docs/overview.md` | Install, Docker, packaging, examples, and benchmarks | docs, examples, benchmarks, README status map, install-packaging boundary docs, public example tests, and generated parity scorecard | Phase 5 | partial |
