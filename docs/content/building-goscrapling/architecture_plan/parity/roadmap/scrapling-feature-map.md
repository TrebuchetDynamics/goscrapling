...
| CLI shell and extract commands | `scrapling/cli.py`, `docs/cli/**` | `cmd/goscrapling`, `internal/cli` | Non-mutating `install` guidance, static `extract get/post/put/delete`, markdown output, AI-targeted cleanup, fake-backed `extract fetch`/`stealthy-fetch` browser option wiring, scripted `shell -c` get/post/put/delete shortcuts with narrow data/json/header options, help/namespace listing, page/response/pages shortcut behavior, pages history expression shortcuts, view(page)/view(response) artifact writing, and curl helper parsing/execution are fixture-backed. The next bounded shell gap is explicit fetch/stealthy_fetch browser shortcut boundaries; keep full REPL behavior and richer shell expressions as future delta rows. | Phase 5 | partial |
| MCP and AI docs | `docs/ai/mcp-server.md`, `docs/api-reference/mcp-server.md`, `scrapling/core/ai.py` | `integrations/mcp`, future integration packages | Deterministic MCP-style get, bulk_get, fetch, bulk_fetch, stealthy_fetch, bulk_stealthy_fetch, screenshot, open_session, close_session, and list_sessions handlers are fixture-backed with injectable static/browser seams, CSS/main-content extraction, session lifecycle, image/text screenshot blocks, and no live LLM/network dependency. Lightpanda-inspired Gormes rendered markdown, links, semantic tree, structured data, and interactive/form discovery tool taxonomy is fixture-backed. SearXNG-inspired Gormes extraction recipes cover reusable request templates, method/header/cookie/body request controls, paging/status placeholders, CSS/XPath/slash-delimited JSON field extraction, URL/thumbnail prefixes, HTML-to-text transforms, and suggestion extraction as a non-vendored design reference. The Gormes static provider registry covers integration-local provider metadata, shortcuts, categories, enablement, timeouts, weights, and deterministic provider selection without live federation; the result container covers duplicate URL merging, provider/position evidence, weight-aware ordering, suggestions, and unresponsive-provider diagnostics without live multi-provider execution. The planned provider failure diagnostics row narrows SearXNG-style processor error handling to deterministic provider error/suspended evidence without retries or scheduler behavior. | Phase 5 | partial |
