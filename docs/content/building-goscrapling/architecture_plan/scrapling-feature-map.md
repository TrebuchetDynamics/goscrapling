...
| Install, Docker, packaging, examples, and benchmarks | `Dockerfile`, `pyproject.toml`, `docs/benchmarks.md`, `docs/tutorials/**`, `docs/overview.md` | `cmd/goscrapling`, `docs/`, `benchmarks/` | README now maps major upstream docs/product groups to Go status or exclusion, and `TestExamples` compiles public parser/fetcher examples. Planned rows still cover benchmarks, generated parity scorecards, install command behavior, Docker image details, and deeper migration/API reference docs. | Phase 5 | partial |
