...
| CLI shell and extract commands | `scrapling/cli.py`, `docs/cli/**` | `cmd/goscrapling`, `internal/cli` | Non-mutating `install` guidance, static `extract get/post/put/delete`, markdown output, AI-targeted cleanup, fake-backed `extract fetch`/`stealthy-fetch` browser option wiring, and scripted `shell -c` get/page shortcut behavior are fixture-backed. Scripted shell curl helper parsing/execution is also fixture-backed. The next bounded CLI gap is post/put/delete shell method shortcuts; keep full REPL behavior and richer shell expressions as future delta rows. | Phase 5 | partial |
...
4. Treat tool surfaces as delta-driven after the completed CLI extract,
   install, MCP, and Gormes rows; split any the planned shell static-method shortcut row, deeper shell/packaging gaps, or
