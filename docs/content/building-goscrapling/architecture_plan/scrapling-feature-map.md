...
| Browser fetching | `docs/fetching/dynamic.md`, `docs/fetching/stealthy.md`, `scrapling/fetchers/chrome.py`, `scrapling/fetchers/stealth_chrome.py`, `scrapling/engines/_browsers/**`, `scrapling/engines/toolbelt/ad_domains.py` | root package `BrowserFetcher`, `ChromedpBrowserEngine`, future browser/session adapters | Engine-neutral contract and real chromedp-backed JavaScript rendering are covered. Planned rows still cover session/page pools, deeper wait/actions/downloads/screenshots/XHR behavior, context options, resource/ad blocking, stealth controls, and Cloudflare-claim boundaries. | Phase 3 | partial |
| Spider runtime | `docs/spiders/**`, `scrapling/spiders/**` | `spider` package | Request/result/session/scheduler contracts and allowed-domain offsite filtering are fixture-backed. Planned rows cover engine concurrency, robots, cache, checkpointing, blocked retries, lifecycle/streaming/stats/export, link extraction/templates, and session adapters. | Phase 4 | partial |
