...
|---|---|---|---|---|---|
| phase-0-parser-adaptive / adaptive-storage | Adaptive selector fallback and auto-save modes | `parser` | `medium` | `draft` | P0 port parity row with complete handoff metadata. |
| phase-0-parser-adaptive / static-document | Selector pseudo-elements, extraction helpers, and custom types | `parser` | `medium` | `draft` | P0 port parity row with complete handoff metadata. |
| phase-0-parser-adaptive / adaptive-storage | Adaptive fingerprint parity review and diagnostics | `parser` | `medium` | `draft` | Contract metadata is present and the row is unblocked. |
