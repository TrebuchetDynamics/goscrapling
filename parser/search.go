...
func (e *Element) FindSimilar(opts SimilarOptions) Selection {
	...
	for _, candidate := range allDescendantElements(e.doc, e.doc.root) {
		...
		if opts.MatchText {
			score = (score + storage.StringSimilarity(targetText, directElementText(candidate.node, true))) / 2
		}
		if score >= threshold {
...
func attributeSimilarity(left map[string]string, right map[string]string) float64 {
	return (storage.MapKeySimilarity(left, right) + storage.AttrValueSimilarity(left, right)) / 2
}
